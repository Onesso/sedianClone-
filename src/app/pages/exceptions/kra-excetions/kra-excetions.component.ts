...
@Component({
  selector: 'app-kra-excetions',
  templateUrl: './kra-excetions.component.html',
  styleUrls: ['./kra-excetions.component.scss'],
  providers: [ExceptionService, DecimalPipe]
})
export class KraExcetionsComponent {
 // bread crumb items
 ...
 getIprs() {
   ...
   this.compliance.filterExceptions("KRA", "", "").subscribe(
     {
       next: (resp: any) => {      
 ...
 getIprsById() {
  ...
  const startIndex = (this.currentPage - 1) * this.pageSize;
  const endIndex = startIndex + this.pageSize;
  this.compliance.filterExceptions("KRA", this.search, this.search).subscribe(
    {
      next: (resp: any) => {      
          switch (resp.messageCode) {
            case '00':
 ...
 viewCustomer(data: any){
  ...
  if(this.exceptionLevel === 'authorize') this.router.navigate(['app/exception/kra-details', data.customerNo,data.groupCode]);
  if(this.exceptionLevel === 'approve') this.router.navigate(['app/exception/pending-approval', 'KRA', data.customerNo,data.groupCode]);
 }
...
exportexcel(): void
{
  ...
  const fileName = "KRA Exceptions.xlsx";
