...
@Component({
  selector: "app-t24-detals",
  templateUrl: "./t24-detals.component.html",
  styleUrls: ["./t24-detals.component.scss"],
  providers: [DecimalPipe, SettingsService, ExceptionService],
})
export class T24DetalsComponent {
  // bread crumb items
        ...
        .subscribe((resp: any) => {
          switch (resp.messageCode) {
            case "00":
              ...
              this.name = `T24 EXCEPTION FOR ${this.details[0]?.customerName}`;
              if (imagesData.frontId)
    ...
    this.service.authorizeForAccountCreation(payload).subscribe((resp: any) => {
      switch (resp.messageCode) {
        case "00":
          ...
          setTimeout(() => {
            this.router.navigate([`/app/exception/t24/${this.level}`]);
          },2000)
          break;
        case "01":
          this.toastr.info(resp.message);
