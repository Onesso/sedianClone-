...
import { Cart } from './iprs-details.model';
...
@Component({
  selector: 'app-cart',
  templateUrl: './iprs-details.component.html',
  styleUrls: ['./iprs-details.component.scss'],
 ...
 * Cart Component
 */
export class IprsDetailsComponent implements OnInit {
        ...
        .subscribe((resp: any) => {
          switch (resp.messageCode) {
            case "00":
              ...
              this.name = `IPRS EXCEPTION FOR ${this.details[0]?.customerName}`;
              ...
              this.photoImages.push(BACKitem);
               ...
               this.images.push(SELitem);
                      ...
                      //Iprs Image
              const IprsSrc = resp.data.images[0].iprsImage ?? 'assets/images/profile.jpg';
              const IPRScaption = 'IPRS Image';
              const IPRSthumb =  resp.data.images[0].iprsImage?? 'assets/images/profile.jpg';
              const IPitem = {
                src: IprsSrc,
                caption: IPRScaption,
                thumb: IPRSthumb
    ...
    this.service.authorizeForAccountCreation(payload).subscribe((resp: any) => {
      switch (resp.messageCode) {
        case "00":          
          ...
          setTimeout(() => {
            this.router.navigate([`/app/exception/iprs/${this.level}`]);
          },2000);             
          break;
        case "01":
    ...
    this.service.clearException(payload).subscribe((resp: any) => {
      switch (resp.messageCode) {
        case "00":          
          ...
          this.inviteCustomer();
          this.toastr.success("Customer has been invited to visit their nearest Sidian branch for assistance.");
          setTimeout(() => {
            this.router.navigate([`/app/exception/iprs/${this.level}`]);
          },2000);          
          break;
