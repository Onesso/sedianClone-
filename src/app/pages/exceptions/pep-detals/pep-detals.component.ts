...
@Component({
  selector: 'app-pep-detals',
  templateUrl: './pep-detals.component.html',
  styleUrls: ['./pep-detals.component.scss'],
  providers:[DecimalPipe, SettingsService, ExceptionService]
})
export class PepDetalsComponent {
        ...
        .subscribe((resp: any) => {
          switch (resp.messageCode) {
            case "00":
              ...
              this.name = `PEP EXCEPTION FOR ${this.details[0]?.customerName}`;
              ...
              this.photoImages.push(BACKitem);
               ...
               const SELsrc = imagesData?.selfieImage? imagesData.selfieImage: 'assets/images/profile.jpg';
               this.selfie = SELsrc;            
               const SELcaption = 'Selfie'
               const SELthumb =  imagesData.selfieImage? imagesData.selfieImage: 'assets/images/profile.jpg';
               const SELitem = {
                 src: SELsrc,
                 caption: SELcaption,
                 thumb: SELthumb
    ...
    this.service.authorizeForAccountCreation(payload).subscribe((resp: any) => {
      switch (resp.messageCode) {
        case "00":          
          ...
          setTimeout(() => {
            this.router.navigate([`/app/exception/pep/${this.level}`]);
          },2000)
          break;
        case "01":
          this.toastr.info(resp.message);          
    ...
    this.service.clearException(payload).subscribe((resp: any) => {
      switch (resp.messageCode) {
        case "00":        
          ...
          this.toastr.success('Customer has been notified to visit their nearest Sidian branch for help.');
          setTimeout(() => {
            this.router.navigate([`/app/exception/pep/${this.level}`]);
          }, 2000);  
          break;
