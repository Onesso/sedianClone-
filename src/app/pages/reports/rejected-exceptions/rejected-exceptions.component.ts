...
@Component({
  selector: 'app-rejected-exceptions',
  templateUrl: './rejected-exceptions.component.html',
  styleUrls: ['./rejected-exceptions.component.scss']
})
export class RejectedExceptionsComponent {
  ...
  ngOnInit(): void {
    ...
    */
     this.breadCrumbItems = [
      { label: 'Reports' },
      { label: 'Rejected Reports', active: true }
    ];
    this.fetchReport();
  }
...
formatDate(date: any) {
    ...
    this.service.getExceptionReport(startIndex.toString(), endIndex.toString(), "","").subscribe((resp: any) => {
      switch (resp.messageCode) {
        case '00':
          ...
          this.approved =this.rows.filter(data => data.approveRejected === 'REJECTED');          
  ...
  exportexcel(): void
  {
    ...
    const fileName = "Rejected Exceptions List.xlsx";
